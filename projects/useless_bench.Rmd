...
I had recently scraped NBA box score data for one of my other projects, and it was almost perfect for my purposes today. I had taken basic box score data from all games starting from the 1996-1997 season to December 31, 2020, and aggregated them all into a .csv file (available on Kaggle). The only adjustment I had to make was to write a quick macro to determine whether players were starters, but otherwise the data was already good to go.

My thought process was to see how many games in my dataset saw bench production as bad or worse than the Nets, in terms of several metrics and filters. For the final notebook, I arranged my findings in rough order of increasing specificity.
...
What if we add the restriction that at least four players actually playing off the bench?
...
```{r, echo = FALSE}
starters_data %>% 
    filter(TEAM == "DAL", ReservePoints / TotalPoints < 0.1, ReserveMinutes / TotalMinutes > 1/4, Reserves >= 4) %>% 
...
```{r, echo = FALSE}
starters_data %>% 
    filter(Year > 2011, ReservePoints / TotalPoints < 0.1, ReserveMinutes / TotalMinutes > 1/4, Reserves >= 4) %>% 
    group_by(TEAM) %>% 
    summarize(COUNT = n()) %>% 
    arrange(COUNT)
```
...
```{r, fig.width = 10}
starters_data %>% 
    filter(ReservePoints / TotalPoints < 0.1, Reserves >= 4, ReserveMinutes / TotalMinutes > 1/4) %>% 
    select(Year) %>%  
    ggplot(aes(x = as.numeric(Year))) + 
    stat_bin(fill = "orange", color = "black", breaks = 1995:2020) + 
    labs(title = "Bench Scoring <10% of Points in >25% of Minutes (Min 4 Players)",
         x = "Year", y = "Count")
```

Although the occurrences are decreasing in recent years, it's a bit jarring to see that quite often teams' starters have to carry their teams so hard. For instance, the Celtics bench played 4 such games in 2020 alone. Of course, we can probably expect the Nets to see a few more such games this year.

```{r, echo = FALSE}
starters_data %>% 
    filter(Year > 2019, ReservePoints / TotalPoints <= 0.1, Reserves >= 4, ReserveMinutes / TotalMinutes > 1/4)
```
